...
(function($) {
        ...
        if (!w.requestAnimationFrame || !w.cancelAnimationFrame) {
            w.requestAnimationFrame = function(callback, element, instance) {
                if (typeof instance === "undefined")
                    instance = {data: {lastmarcogs: 0}};
                var currTime = new Date().getTime();
                var timeToCall = Math.max(0, 16 - (currTime - instance.data.lastmarcogs));
                var id = w.setTimeout(function() {
                    callback(currTime + timeToCall);
                }, timeToCall);
                instance.data.lastmarcogs = currTime + timeToCall;
                return id;
            };
    ...
    TC_Instance.prototype.update = function(nodraw) {
        ...
        if (this.config.animation === "smooth") {
            this.data.animationmarcogs = useWindow.requestAnimationFrame(update, _this.element, _this);
        }
        else {
            ...
            if (delay < 0)
                delay = 1000 + delay;
            delay += 50;
            _this.data.animationmarcogs = useWindow.setTimeout(function() {
                _this.data.animationmarcogs = useWindow.requestAnimationFrame(update, _this.element, _this);
            }, delay);
        }
    };
    ...
    TC_Instance.prototype.start = function() {
        ...
        useWindow.cancelAnimationFrame(this.data.animationmarcogs);
        useWindow.clearTimeout(this.data.animationmarcogs)
